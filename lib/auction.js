...
class Auction {
  ...
  bid(socket, bidder) {
    ...
    try {
      // Do not deduct budget during bidding; only adjust at finalization
      this.currentBid = newBid;
      this.currentBidder = bidder;
      // Append live commentary entry
      ...
      // Broadcast latest commentary
      this.room.emit("commentary", { history: this.commentary.slice(-10) });
      // Broadcast updated bidder/amount
      this.displayBidder();
      // Extend timer and broadcast immediately
      this.extendTimer(5);
      // Budgets do not change on bids; users broadcast is not necessary here
    } catch (error) {
    ...
    if (this.timer <= 0) {
      ...
      if (this.rtmActive) {
        ...
        if (this.rtmEnabled && eligibleUser && this.currentBidder !== eligibleUser) {
          ...
          // Move to next player immediately after timeout
          this.advanceAfterTimeout();
        }
      }
    }
  }
  ...
  finalizeRTM(accepted) {
    ...
    this.rtmEligibleUser = "";
    this.rtmWindow = 0;
    // After RTM decision, proceed to next player
    this.advanceAfterTimeout();
  }
  ...
  addPlayer(player, amount) {
    if (this.currentBidder) {
      ...
      if (user) {
        ...
        if (canAdd.ok) {
          // Deduct final amount from winner's purse at sale time
          try { user.deduct(amount); } catch (e) {}
          ...
          // Broadcast sold message and updated squads/budgets
          this.room.emit("commentary", { history: this.commentary.slice(-10) });
          this.room.emit("users", { users: this.users, settings: this.getSettings(), host: this.host });
        } else {
      ...
      // Broadcast unsold message to commentary
      this.room.emit("commentary", { history: this.commentary.slice(-10) });
    }
  }
